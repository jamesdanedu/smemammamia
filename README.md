...
7:30 PM. €10 flat, 504 seats a night, at St Mary's Secondary School, Edenderry,
...
seating.js         >>> The gym floor: hall size, rows, blocks, aisles — the 504 seats
...
The plan as it stands — **18 rows of 28 seats, 504 a night**:

| | |
|---|---|
| Hall floor | 30.78 m × 18.90 m (101 ft × 62 ft, measured) |
| Stage | 10.67 m (about 35 ft) deep across one end, 1.50 m clear in front of row A |
| Rows | 18, at 0.85 m — lettered A to T, skipping I and O |
| Seats across | 7 + 14 + 7, with two 1.20 m aisles |
| Side gangways | 1.25 m at each wall |
...
gangway is at least 1.05 m. Four seats on the cross gangway (K1, K2, K27, K28) are
wheelchair spaces — the chair is taken away rather than the seat deleted, so the
count stays at 504 and the space can still be sold if nobody needs it.
...
| 2 | **504 capacity per night** — check it against the hall's fire cert before selling to it. | `config.js`, `seating.js`, the seed block in `schema.sql`, admin *Settings* |
| 2a | **The stage depth.** The hall is measured at 101 ft × 62 ft; the plan gives the stage 35 ft of the length. If the stage comes out further, every 0.85 m (about 2 ft 9 in) costs a row of 28 seats, and there is only 12 cm to spare — change `LAYOUT.stage.depthM` in `seating.js` and check `seating.html` still reports no problems. | `seating.js` |
...
* **Capacity and `seating.js` disagreeing.** Capacity above 504 sells tickets there
  is nowhere to seat; below 504 leaves seats that cannot be sold.
